...
test('DTLA theme uses a technical-ledger typography stack instead of cyberpunk display fonts', async () => {
...
test('DTLA theme reduces cockpit glow/noise into a restrained ledger surface', async () => {
  const theme = await readFile(THEME_PATH, 'utf8');
...
test('DTLA plugin chrome uses muted technical labels, not neon HUD treatment', async () => {
  ...
  assert.doesNotMatch(css, /text-shadow|dtlaPulse|dtlaSweep|scanline|neon/i);
});
