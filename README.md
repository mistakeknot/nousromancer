...
The theme keeps the dashboard in the standard layout to avoid sidebar collisions with other installed plugins. The companion plugin adds a persistent Now Bar above every page: gateway health, active runs, latest trace, latest session source, data freshness, conservative `Possibly waiting` hints, and the next useful action in one restrained strip. It distinguishes live gateway, stale refresh, API-error states, source context such as `src:discord`, assistant-question hints, and session error/stall evidence in text instead of color alone. It also adds a small header crest and night-ops status pill.
...
- A persistent Now Bar that keeps gateway health, active runs, latest trace, latest session source, data freshness, API-error state, conservative `Possibly waiting` hints, and the next useful action visible across routes.
